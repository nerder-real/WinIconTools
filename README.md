...
| 分区 | 功能 | 说明 |
|---|---|---|
| 隐藏快捷方式小箭头 | 方案一（推荐） | `explorer.exe,-264` 系统内置空图标，零文件部署 |
| | 方案二 | `Taskbar.dll,-264` 备用方案 |
| | 方案三（备选） | 部署全透明 `blank.ico` |
| | 自定义方案 | 查看当前注册表参数（识别其它工具的设置） |
| 隐藏「- 快捷方式」后缀 | 方案一 | `Explorer\Link = 00000000`，仅对**新建**快捷方式生效 |
| | 自定义方案 | 查看 `Link` / `Naming Templates` 参数 |
| 隐藏 UAC 小盾牌 | 方案一（推荐） | 全透明 `blank.ico`，无残留边框 |
| | 方案二 | `shell32.dll,-50` |
| | 方案三（备选） | `explorer.exe,-264` |
| | 自定义方案 | 查看当前注册表参数 |
| 恢复系统默认 | 一键还原 | 小箭头 / 后缀 / UAC 盾牌 独立恢复 |
| 系统实用工具 | Win10 经典右键菜单 / Win11 新版右键菜单 | 切换 CLSID `{86ca1aa0-...}`，支持强删 HKLM 中 TrustedInstaller 所有的键 |
| | 重建桌面图标缓存 | 结束 Explorer → 清缓存 → 看门狗自动拉起 |

**状态卡**：四行实时状态（名称 + 彩色状态点 + 右侧徽标），哪个方案生效哪个按钮亮，一目了然。

## 📦 下载与使用

1. 从 [Releases](../../releases) 下载 `桌面图标美化工具.exe`；
...
![预览](screenshots/preview.png)（可自行截图替换）

- 无边框圆角窗口，深色/浅色模式实时跟随系统（2 秒内自动换肤）
- 方案按钮：选中实心渐变高亮，未选中描边
